...
the drill tool — it talks to it only as a subprocess, exactly like the rest of
drillui. It can drive EITHER pdfdrill OR htmldrill (the two expose the same verbs
drillui needs — retrieve / chatlog / model / status). The tool is chosen by
``--tool`` or auto-detected from the doc address (.html/http/file → htmldrill,
.pdf/.md/arxiv-id → pdfdrill). For each user question it:

  1. asks the tool to TRANSFORM the question into grounded context
     (`<tool> retrieve <doc> "<q>" --json` → the top-k relevant drilled units +
     a ready prompt that cites them by id — the question-transformation step);
  2. PROXIES that enriched prompt to an LLM via a headless
     `claude -p ... --output-format json`;
  3. stores the Q&A back (`<tool> chatlog … → a transcript line`).

So the *knowledge* (retrieval + storage) lives in the drill tool and is reused;
only the conversation loop + the LLM call live here.

Usage:
    drillui_chat.py <doc> [-q "question"]      # one-shot, or a REPL with no -q
    drillui_chat.py <doc> --tool htmldrill     # force the htmldrill backend
    drillui_chat.py <doc> --once status        # NON-INTERACTIVE: run ONE verb, exit
    drillui_chat.py <doc> --model claude-haiku-4-5-20251001 --k 10

SMOKE-TEST SCOPE (honest): ``--once <verb>`` is a launch + single-dispatch smoke
path — it resolves the tool, runs ONE drill subcommand against the doc, prints
its prose, and exits 0. It does NOT exercise the interactive TUI, readline, the
multi-doc `add`/combine flow, or the LLM call (`claude -p`). Those remain
untested by the smoke check by design (the LLM path needs a live `claude`).

NOTE on chatlog flags: pdfdrill's chatlog takes ``--question``; htmldrill's takes
``--ask``. The LLM REPL/-q path's ``chatlog()`` proxy adapts to the resolved tool.
...
# htmldrill checkout — drillui can drive EITHER tool. htmldrill exposes the same
# verbs drillui needs (retrieve / chatlog / model / status), so once `base`
# resolves to htmldrill the rest of this file is unchanged. (htmldrill has no
# `combine`/`skill --json`; load_commands degrades gracefully to {} and do_add's
# combine path is only hit on multi-doc `add`, so single-doc chat works.)
_HTMLDRILL_DEFAULT = Path(os.environ.get("DRILLUI_HTMLDRILL") or "/home/wkolbe/htmldrill")


def _detect_tool(doc: str | None) -> str:
    """Auto-detect which tool a document belongs to from its address.

    An http(s)/file URL or a .html/.htm file → htmldrill; a .pdf / .md / bare
    arxiv-id → pdfdrill. Mirrors the existing doc.lower().endswith logic. Defaults
    to pdfdrill when there's no document (the historical behaviour)."""
    if not doc:
        return "pdfdrill"
    d = doc.lower()
    if d.endswith((".html", ".htm")) or d.startswith(("http://", "https://", "file://")):
        return "htmldrill"
    return "pdfdrill"


def _pdfdrill_cmd(args: argparse.Namespace) -> tuple[list[str], dict, str]:
    """Resolve how to invoke the drill tool. Returns (argv_base, env, description).

    Branches on the resolved tool (explicit --tool, else auto-detected from the
    doc): tool=='htmldrill' → `python -m htmldrill` with PYTHONPATH=<htmldrill>/src;
    otherwise the unchanged pdfdrill order — --pdfdrill override > --src > in-repo
    src tree > ./pdfdrill wrapper > an installed `pdfdrill` on PATH.
    """
    import shutil
    env = dict(os.environ)
    ...
    tool = getattr(args, "tool", None) or _detect_tool(getattr(args, "doc", None))

    def _with_src(src: Path, pkg: str) -> tuple[list[str], dict, str]:
        e = dict(env)
        e["PYTHONPATH"] = str(src) + os.pathsep + e.get("PYTHONPATH", "")
        return [sys.executable, "-m", pkg], e, f"python -m {pkg} (PYTHONPATH={src})"

    if tool == "htmldrill":
        if args.pdfdrill:                              # explicit override still wins
            return args.pdfdrill.split(), env, f"override: {args.pdfdrill}"
        if args.src:
            return _with_src(Path(args.src).resolve(), "htmldrill")
        for root in (_REPO_ROOT, _HTMLDRILL_DEFAULT):
            src = root / "src"
            if (src / "htmldrill" / "__init__.py").exists():
                return _with_src(src, "htmldrill")
        found = shutil.which("htmldrill")
        if found:
            return ["htmldrill"], env, f"installed: {found}"
        raise SystemExit(
            f"error: could not locate htmldrill. Set $DRILLUI_HTMLDRILL to a "
            f"checkout, or pass --src <htmldrill/src>.")

    if args.pdfdrill:                                  # explicit override
        return args.pdfdrill.split(), env, f"override: {args.pdfdrill}"
    if args.src:                                       # explicit --src DIR
        return _with_src(Path(args.src).resolve(), "pdfdrill")

    src = _REPO_ROOT / "src"
    if (src / "pdfdrill" / "__init__.py").exists():    # the obvious answer
        return _with_src(src, "pdfdrill")
...
def chatlog(base: list[str], env: dict, doc: str, q: str, a: str,
    ...
    # htmldrill's chatlog takes --ask; pdfdrill's takes --question. Pick by the
    # resolved backend (base ends with the tool's module/cmd name).
    is_html = any("htmldrill" in part for part in base)
    q_flag = "--ask" if is_html else "--question"
    return _run(base + ["chatlog", doc, q_flag, q, "--answer", a,
...
def main() -> int:
    ...
    ap.add_argument("--tool", choices=["pdfdrill", "htmldrill"], default=None,
                    help="which drill tool to drive (default: auto-detect from the "
                         "doc — .html/http/file → htmldrill, .pdf/.md → pdfdrill)")
    ap.add_argument("--once", metavar="CMD",
                    help="NON-INTERACTIVE: run ONE drill subcommand against the doc "
                         "and print its prose, then exit (e.g. --once retrieve, "
                         "--once status). No LLM, no TTY. A smoke-test entry point.")
    ...
    # --once: a single, non-interactive subcommand dispatch. This is the smoke-test
    # path — it does NOT exercise the interactive TUI or the LLM call; it only
    # confirms the tool resolves and ONE verb runs end-to-end and returns prose.
    # The doc is auto-inserted as the first positional (every htmldrill verb takes
    # a <url>); extra words after the verb (e.g. a retrieve query) ride through.
    if args.once is not None:
        if args.doc is None:
            print("error: --once needs a <doc>.", file=sys.stderr)
            return 2
        parts = args.once.split()
        if not parts:
            print("error: --once needs a subcommand (e.g. --once status).", file=sys.stderr)
            return 2
        verb, rest = parts[0], parts[1:]
        try:
            out = _run(base + [verb, args.doc, *rest], env, timeout=args.timeout)
            print(out.rstrip())
            return 0
        except Exception as e:
            print(f"error: {e}", file=sys.stderr)
            print(f"  (drill tool via {how})", file=sys.stderr)
            return 1
